...
- ☠️ **A lever that changes state cannot have a return leg** — after B the world contains B, so the third leg is a second B, not an A′. Do not label it A‑B‑A′ anyway: the name is what a later reader trusts, and a returnless comparison wearing that name claims a control it never had. Run a **gated before/after** instead, and put the gate where the experiment is easiest to fake — here, on whether leg A is already in the state B is supposed to produce. A B run against an already-satisfied A yields exactly the hoped-for number while measuring nothing, and its output looks indistinguishable from a real result. The counterfactual then has to come from a separate occasion (the same protocol, the state absent, the lever not pulled), and if it does not exist yet, that belongs in the write-up rather than around it.
...
- ☠️ **A count over a window is worth exactly what BOTH its edges are worth, and the near edge is the one you forget.** A trace was cut at the resume so the wake storm could not contaminate it — and the far edge was the only one anyone thought about. Tracing had been switched on *before* the suspend call, so the buffer also held the traffic that the suspend call itself provokes: the power daemon's own handshake with the modem, complete with replies. It was counted as "packets that arrived while asleep", which inverted the finding — that traffic was **ours**, and whether the device sends anything unprompted was never measured. Mark both edges from inside the run, decode only between them, and **refuse to report at all when a marker is missing**, because a half-bounded window looks exactly like a bounded one.
- ☠️ **When your own tooling acts on the system, its actions are inside the window too.** Ask, before reading any count: what did *starting this measurement* cause? A suspend request runs a daemon; a module load runs a probe; a service restart re-registers everything it had registered. The give-away is a per-round count that is **identical every round** — network traffic varies, a fixed handshake does not. Read a suspiciously regular count as a signature of your own procedure until proven otherwise.
- Every measurement needs a real path to PASS, stated in advance.
- ☠️ **Before measuring, run the prior-art check — as a command, not as an intention.** The prose form of this rule failed the same day it was written: a session added "grep the captures for the field, not the topic" to this list in the morning and, four hours later, measured a counter across a fresh window, wrote the conclusion, and only then found the branch closed eleven days earlier by an A/B that moved that counter by two thousand and the outcome by 0.4 %. **A rule in prose is a wish; a rule you can run is a rule** — on this port that is `fp3-pmaports/docs/power/bringup/tools/prior-art.sh <field>`. Search by the field or counter name, never the topic, because captures are indexed by date and by the question that prompted them. And treat a null result as a spelling question first: the same quantity is often named differently downstream and upstream, and the miss looks exactly like an absence.
- ☠️ **A multi-channel instrument read as a single-channel one hides its own answer.** When a tool prints several channels and you came for one of them, the others are still measurements and they are still yours to read. An unexpected saturated value in a column you were not looking at — 0 %, 100 %, a counter that never moves — deserves the same weight as the number you came for, and usually more, because nobody chose to look at it. Worked example: a four-processor duty instrument printed `LPASS awake 100.0 %` on every run for **three days** while attention stayed on the modem row, because the question was "the modem". The oracle side of the very same captures read 2.8–3.0 %, so the 100 % was a real two-sided differential sitting in a file that had been opened repeatedly. **Read every column the tool prints, every run**, and when one is saturated, decide explicitly whether it is a finding or an artefact before moving on — the same counter reading a non-saturated value on the reference side settles that in one grep.
- ☠️ **A lever has preconditions of its own, and a lever that never fired is not a negative result about the system.** Setting the knob is not the same as the knob acting. Read the governing document for the *conditions* under which the mechanism you just enabled actually does anything, and capture a witness for **each** one — otherwise "nothing happened" is indistinguishable from "the thing never got as far as trying", and the first reads as a discovery about the hardware while being a fact about your setup. Worked example: an experiment set logind's `IdleAction=suspend` to ask whether a phone would suspend on its own. `logind.conf(5)` states three conditions — *"after all sessions report that they are idle, no idle inhibitor lock is active"*, and the delay has expired — and the script captured only the inhibitor list. A graphical session with the display on may never report idle at all, so a zero would have been written up as "the phone refuses to suspend" when it meant "the policy was never consulted". The missing witness cost one line (`loginctl show-session -p IdleHint`); the wrong conclusion would have cost a night and pointed the next day's work at the wrong subsystem. **State each precondition, name the witness that shows it held, and if a witness does not exist, say the result is conditional on it.**
...
- ☠️ **Before reporting that a tool lacks something, read its own usage text — not just the code path you are standing in.** A missing-feature report is a claim about the *whole* tool, so it takes the whole tool to justify, exactly as "new here" is a claim about the whole source tree. Worked example: a session spent six turns being nagged by an autonomy hook, wrote up "it has no waiting state" as a defect, and proposed a fix — while the hook's own header documented `wait <id> "<what>"`, rendered such items distinctly, and carried a comment in that very branch explaining it announces them once and then stays silent. The stall-detection code had been read; the usage block twenty lines above it had not. `--help`, the header comment, the list of accepted subcommands: all three are cheaper than the report.
...
- ☠️ **A confound can be a regime, not a bias.** The same A/B ran with the display refreshing and with it off, and the two gave *opposite signs* - both measured correctly. At 65 wakeups a second the idle windows never got long enough for the deep state to repay its entry cost; with the panel off they did. No amount of extra samples or interleaving finds this, because neither arm is wrong. Only gating the precondition and letting the gate fail loudly does.
- ☠️ **Check the system ever enters the state your number is supposed to describe.** A figure called "the idle baseline" was measured, A/B'd, written up and queued for a subsystem bisect before anyone read `/sys/power/suspend_stats/success` and found it was `0` — the phone had never suspended once, so every number described runtime idle with a full desktop session and a chatty modem, not the regime the target figure comes from. The regime a name implies is a claim like any other: find the counter that says the state was entered, and read it *before* optimising anything against it. The cost of not doing so is not a wrong number — it is a correct number about the wrong thing, which survives review because nothing about it looks false.
- ☠️ **A unit is not a mechanism — find out how the number is produced before integrating it.** An attribute in µAh looks like a coulomb count and invites `(after - before) * 3600 / seconds` as a current. On a platform with no hardware gauge it can instead be a *voltage* looked up in an OCV table by a 30-second poll worker, in which case that arithmetic measures the estimator's own settling and, across a suspend, measures nothing at all because the worker is frozen. One `grep` for the provider (`drivers/power/supply/adc-battery-helper.c` here, whose header says outright what it is for) costs less than the write-up of a wrong result. The general form: a plausible unit on a sysfs attribute is a hint about intent, never about derivation.
- ☠️ **A generated command is evaluated twice, and the first evaluation is invisible.** Anything you build as a string - a heredoc written by a script, an `ssh` payload, a `git commit -m` message - passes through one shell before the one you meant. Measured three times on 2026-08-31, in three different disguises: an unquoted heredoc expanded `$(cat /sys/power/suspend_stats/success)` at *write* time, so a before/after pair became one reading printed twice and looked exactly like a counter that never moved; `cmd | tail -25` followed by `rc=$?` reported `tail`'s status, so a `meson setup` that died on a missing dependency printed `rc=0`; and backticks inside a double-quoted `-m` ran as a command substitution and left a hole in the record. The general form: **a value that was captured earlier than you think, and a status that came from a later process than you think, both look like ordinary results.** Quote the heredoc delimiter (`<<'EOS'`) unless you *want* host-side expansion, read `${PIPESTATUS[0]}` or drop the pipe, and prefer `-F -` with a quoted heredoc over `-m "…"` for anything containing backticks or `$`.
- ☠️ **Put a control window in the regime you already have a number for.** The suspend measurement above ran an awake window through the same instrument purely as a control, and that control read 209 mA against a known 130 mA — which is the only reason the asleep reading was not published as a spectacular sub-2 mA result. A new instrument aimed solely at the regime nothing can cross-check is unfalsifiable by construction; give it one window whose answer you can already look up.
- ☠️ **Prove the wakeup source before you let it end an unattended measurement.** A leg that suspends the device is only safe if something is guaranteed to bring it back, and "the RTC alarm should work" is not that guarantee — on a platform whose RTC cannot be *set* it is a reasonable guess that it also cannot alarm. Run the trivial version first (short alarm, `echo mem`, confirm `success` advanced and the elapsed time matches what you asked for), and only then build the long leg on it. Here the guess was wrong in the safe direction — the clock is stuck in 1970 for want of an `offset` nvmem cell, but an alarm is relative to the counter, so it fires — and that is exactly the kind of thing only the probe can tell you.
- ☠️ **Stopping the service is not stopping the hardware.** Killing the compositor left the panel refreshing at 65 Hz - `fbcon` holds DRM DPMS on with no userspace client at all - so an entire night of idle-current A/B ran under a load that dwarfed the effect being measured, and the honest-looking negative result was about the display. Before measuring idle anything, verify the *device* is idle at its own counters (here `msm_mdss` in `/proc/interrupts`, and `card0-DSI-1/dpms`), not that the process you know about has exited.
...
- ☠️ **A sysfs attribute is either sampled when you read it or served from a cache, and nothing on the surface tells you which.** Open the driver and find the property handler before you build anything on the number. On one battery driver `VOLTAGE_NOW` and `CURRENT_NOW` hit the ADC on every read while `VOLTAGE_OCV`, `CAPACITY` and `CHARGE_NOW` were three names for one value a 30 s work item maintained — so across a suspend, where that work item does not run, all three reported the state before the freeze. The tell that is *not* a tell: the cached value equalled the live ones through the documented formula to the last digit, which reads exactly like an instantaneous computation and is just a quiet interval.
- ☠️ **Several attributes of one device are usually one measurement wearing several names.** They look like independent opinions and agreeing with each other looks like corroboration. Trace each back to where it is written before treating any two as a cross-check.
- ☠️ **The same failure twice means the next instrument needs its source read first, not its design improved.** Three instruments aimed at one question failed identically — each replaced a discredited attribute with a neighbouring attribute from the same driver, without once opening it. Twenty minutes of reading would have skipped all three. After the second failure of a class, stop designing and go read.
- ☠️ **Prefer a slope over a difference whenever the endpoints are conditioned differently.** Two-point windows inherit every offset at each end — a pack still shedding surface charge, polarisation after a resume transient, a static resistance compensating a moving one — and all of them push the same way. Sampling many times under *identical* conditions and fitting the slope cancels any constant, and calibrating that slope against a directly measured value in a second phase removes the lookup table too.
- ☠️ **A wake-source IRQ number is boot-local; only its name is a fact.** `/sys/power/pm_wakeup_irq` returns a Linux IRQ number, and those are handed out in probe order, so the same number is a different device on the next boot. On one boot here 56 was `pm8xxx_rtc_alarm` and 72 was a **camera** interrupt, while captures written hours earlier on other boots read 72 as "the RTC". Resolve it against the `/proc/interrupts` of the boot that produced it, print the name beside the number **in the capture**, and never label it in the prose afterwards — prose is exactly where a boot-local value gets promoted to a permanent fact and then travels into a summary.
- ☠️ **A unit watcher needs two guards, and they pull opposite ways.** Ending the loop on "not running" (`inactive|failed`) fires *before the unit exists*, because `systemctl show -p ActiveState` answers `inactive` for an unknown unit — a watcher started in the same breath as `systemd-run` exits on its first poll and reports a two-minute run finished in nine seconds. Ending it on "no longer in the running set" fires on an **empty** reply, and the reply is empty exactly when the device is unreachable, which for a sleep or panel-blanking measurement is when it is *working* — that reported a ninety-minute run finished after eleven seconds. Do both: warm up until the unit is seen `active|activating`, then loop until a **definite** finished state, treating empty as "still running". ☠️ Both failures happened in one afternoon because the first fix **swapped** the condition instead of widening it: when writing a correction, ask what the old form got right that the new one now gets wrong.
- ☠️ **A counter read only at the end answers a different question than the one asked.** A window meant to detect *whether anything happened* printed `/sys/power/suspend_stats/success` once, at the close, as an absolute value — on a phone already at 75 suspends that day, "75" afterwards is perfectly compatible with zero inside the window, which was the outcome the instrument existed to find. Snapshot before, print the delta as the answer, and when the delta is zero say so **together with the reason that reading could be false** (here an ssh session left open is itself a sleep inhibitor, and would produce the same zero).
- ☠️ **Pre-registration only works if it is re-read at reading time.** A lead had committed in writing to what each outcome would mean, including a bold note naming which way the reading would be tempted to cut and stating that the *comfortable* outcome was the one that retires the design. The measurement landed, matched the prediction exactly, and was written up within the hour as **supporting** the design it retires. Writing the reading table is the cheap half; the half that actually protects you is opening it again next to the result, before drafting a conclusion.
...
- ☠️ **A check is proved only once it has failed under the harness that will run it**, not only by hand. Proving it standalone establishes the verdict logic; the harness supplies the environment, and a per-user service reached from a root check answers *nothing* rather than *no*. Run the known-broken build through the real runner, and read which of its messages came out.
- ☠️ **A skip message must claim only what was measured.** "The graph has no camera node" and "there is no graph" are different claims, and the first reads as a fact about the device while being a fact about the harness — so it survives review. Give every unreachable-instrument path its own wording, and prove each path fires.
...
- ☠️ **A measurement that equals its own bound measured the bound.** A sleep that lasts exactly as long as its alarm, a capture that fills exactly its buffer, a timeout that fires: in each case the instrument's limit, not the system, produced the number — and it reads like a clean result. Set the bound beyond the answer you expect and confirm the run ended on the system's terms; where you cannot, say the figure is a floor, never a value. The version that cost the most here: months of sleep measurements whose "full" durations were all identical to their alarms, so no run in the whole set had ever observed how long the device would actually sleep.
- ☠️ **Back-to-back legs measure the previous leg.** Where an operation leaves the system disturbed for longer than the leg takes — a resume, a reset, a reprobe, a firmware re-sync — every leg after the first starts inside the disturbance, both arms saturate, and the A/B reports "no difference" for a knob it never tested. **"No difference" and "both arms saturated" produce the same table**, so the recovery gap is a parameter to measure, not to guess: run the same leg repeatedly with no knob at all and watch the metric recover, then set the gap from that curve. Suspect this first whenever an A/B comes out flat on a knob with a plausible mechanism.
- ☠️ **Consistent samples look exactly like a result, and the sample size has to be chosen against the SPREAD, not against the effect you hope to see.** Six legs of an A/B came back 52–63 s and were written up as a null result for the knob under test. Hours later the same knob, same path, same configuration, same day gave 280 / 64 / 601 s — an order of magnitude, from a distribution six samples had no chance of showing. The tell was absent by construction: **tight agreement among few samples is what a narrow distribution and an undersampled wide one both look like.** So measure the spread *before* the A/B, with the knob untouched: run the same leg N times and look at the range. If the range approaches the effect you are hunting, the A/B needs a sample size derived from it — or it will produce a confident verdict on a coin flip. This is the deeper version of the back-to-back rule above: that one is about arms contaminating each other, this one is about a single arm not being one number.
- ☠️ **The experimenter is an uninstrumented wake source.** A button press, a cable, a glance at a screen — none of it appears in `pm_wakeup_irq`, in a trace, or in any log the device keeps, and the resulting wake looks exactly like a spontaneous one. It happened here: a sleep ended by the operator's power button was written up as an unexplained contradiction with another measurement and filed as something to go and investigate; it survived only because they mentioned it. Log every human interaction with the device against the clock, and where an interaction cannot be logged, discard the run rather than explaining its result. The mirror of "your own polling can be the wake source", and harder to catch because the tool cannot see it at all.
- ☠️ **When an effect has an unknown time constant, the recovery interval is the independent variable, not a setting.** Picking any constant gap between legs assumes the answer you are trying to measure, and a small one guarantees every leg after the first samples the previous leg's disturbance. Vary the rest period across rounds, take **exactly one** measurement per round (a second corrupts its own next point), and let the rounds be the curve. Worked here, badly: an instrument written specifically to measure a decay used a ten-second gap between rounds and returned the disturbed regime measuring itself — the trap was reproduced within the hour of being written down, because the gap was the one parameter not thought about.
- ☠️ **Prefer a witness on the other side of the link.** Where the host can see the effect — a USB device dropping and re-enumerating, a link going down, a service disconnecting — its log is a measurement with **no observer effect at all**, while every on-device probe is itself an event in the thing being measured. It is also free and retrospective: the log is already there for the whole history, not just from when you started looking. Check for one before writing any on-device instrument for an intermittent or sleep-related question.
- ☠️ **A state a daemon applies on a bus signal does not happen when you bypass the bus — and does not un-happen either.** Driving a transition by writing the kernel interface directly (rather than through the service that owns the policy) skips whatever that service does on the way in; and because the undo is bound to the matching signal, the state then **sticks into your next leg**. That makes leg *order* a confound in a knob-free A/B: a bypassed leg following a policy leg inherits its state and reads identical to it, which is how an A-B-A-B can produce "the two paths are equivalent" out of data that says the opposite. Restart the daemon between legs, or read the state per leg from its own log, and never trust the leg that follows the one you are comparing it against.
- ☠️ **Before believing any idle, sleep or power figure, enumerate what the DISTRIBUTION set on your behalf.** A distro ships policy in package-owned files that are invisible in both upstream documentation and your own tree — service drop-ins that add command-line switches, gsettings overrides that decide whether the session ever asks to suspend, inhibitor configs that quietly hold sleep off while you are logged in. Each can decide the number you are about to attribute to hardware. The sweep is cheap and belongs *before* the measurement, not after a surprising result: enumerate every drop-in with the package that owns it, read the session's power schema keys as the session user sees them, and list the inhibitors that are actually held. When one turns up, **read the code behind the switch, not its help string** — a help line describes the intent, and the branch it selects is where the behaviour is. The full recipe, and the two cases that paid for this rule, are in `/fp3-porting-debug` ("what the distribution decided for you").
